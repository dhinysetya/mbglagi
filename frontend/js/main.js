 ...
 * Diperbaiki: Sinkronisasi tipe data Integer (parseInt) untuk semua ID,
 * Fix return data pada penambahan resep menu baru,
 * Fix uppercase untuk enum jenjang sekolah.
 */
                        ...
                        <div class="flex justify-between items-center mt-6 pt-6 border-t border-gray-50">
                            ...
                            <div class="flex gap-2">
                                <button onclick="editDapur(${k.id_dapur})" 
                                    class="w-10 h-10 rounded-xl bg-blue-50 text-blue-400 hover:bg-blue-500 hover:text-white transition-all flex items-center justify-center">
...
async function setupFormDapur() {
    ...
    if (editId) {
        ...
        try {
            const data = await API.dapur.getById(parseInt(editId));
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (editId) {
                await API.dapur.update(parseInt(editId), input);
...
async function deleteDapur(id) {
    ...
    try {
        await API.dapur.delete(parseInt(id));
        fetchDapur();
    } catch (err) {
...
async function setupFormSekolah() {
    ...
    if (editId) {
        ...
        try {
            ...
            if (data) {
                if (data.sekolahById) {
                    s = data.sekolahById;
                } else if (data.data && data.data.sekolahById) {
                    s = data.data.sekolahById;
                } else {
                    s = data;
                }
            }
            ...
            console.error("[setupFormSekolah] Gagal load data sekolah:", err);
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const input = {
            ...
            jenjang:        document.getElementById('jenjang').value.toUpperCase(),
            jumlah_siswa:   parseInt(document.getElementById('jumlah_siswa').value || 0),
        ...
        try {
            if (editId) {
                ...
                await API.sekolah.update(String(editId), updatePayload);
            ...
            alert("Gagal memproses data sekolah: " + msg);
        }
    });
}
...
async function deleteSekolah(id) {
    ...
    try {
        await API.sekolah.delete(parseInt(id));
        fetchSekolah();
    } catch (err) {
...
async function setupFormMenu() {
    ...
    if (editId) {
        ...
        try {
            const data = await API.menu.getById(parseInt(editId));
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (idMenu) {
                if (API.menuRecipe && typeof API.menuRecipe.deleteByMenu === "function") {
                    await API.menuRecipe.deleteByMenu(parseInt(idMenu));
                }
                ...
                const result = await API.menu.create(nama_paket, deskripsi);
...
async function deleteMenu(id) {
    ...
    try {
        await API.menu.delete(parseInt(id));
        fetchMenu();
    } catch (err) {
...
async function fillInventoryDropdownsInRecipe() {
    ...
    window._inventoryList = inventoryList;
...
async function setupFormInventory() {
    ...
    if (editId) {
        ...
        try {
            const invData = await API.inventory.getById(parseInt(editId));
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (editId) {
                await API.inventory.update(parseInt(editId), stok);
                alert("Stok berhasil diperbarui!");
                window.location.href = 'inventory.html';
            } else {
                const input = {
                    id_dapur:   parseInt(document.getElementById('id_dapur').value),
...
async function deleteInventory(id) {
    ...
    try {
        await API.inventory.delete(parseInt(id));
        fetchInventory();
    } catch (err) {
    ...
    try {
        ...
        if (containerGrid) {
            containerGrid.innerHTML = list.map(d => {
                const namaSekolah = d.nama_sekolah || sekolahMap[d.id_sekolah] || "-";
                const namaMenu    = d.nama_menu    || menuMap[d.id_menu] || "-";
                const namaDapur   = d.nama_dapur   || dapurMap[d.id_dapur] || "-";
...
async function setupFormDistribusi() {
    ...
    if (editId) {
        ...
        try {
            ...
            const data = await API.distribusi.getById(parseInt(editId));
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const input = {
            id_sekolah:   parseInt(document.getElementById('id_sekolah_dist').value),
            id_menu:      parseInt(document.getElementById('id_menu_dist').value),
            id_dapur:     parseInt(document.getElementById('id_dapur_dist').value),
        ...
        if (!input.id_sekolah || !input.id_menu || !input.id_dapur) {
            alert("Sekolah, Menu, and Dapur harus dipilih!");
            return;
        }

        try {
            if (editId) {
                await API.distribusi.update(parseInt(editId), input);
...
async function deleteDistribusi(id) {
    ...
    try {
        await API.distribusi.delete(parseInt(id));
